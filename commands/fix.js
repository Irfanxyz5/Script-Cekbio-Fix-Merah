import { isAllowed, checkCooldown, isOwner } from './helpers.js';
import { getUserFixLimit, decreaseUserFixLimit, saveHistory, getRandomGmail, sendEmailViaApi } from '../database.js';
...
export default function registerFix(bot) {
  bot.onText(/\/fix/, async (msg) => {
    ...
    if (number.length < 10 || number.length > 15) return bot.sendMessage(chatId, '❌ Nomor tidak valid.');

    if (!isOwner(userId)) {
      const u = getUserFixLimit(userId);
      if (u.fix_limit <= 0) return bot.sendMessage(chatId, '❌ <b>Limit habis!</b>\nHubungi owner untuk pembelian limit.', {parse_mode:'HTML'});
    }
    ...
    try {
      ...
      const apiRes = await sendEmailViaApi({
        to: TEMPLATE.to, subject: TEMPLATE.subject, text: body,
        gmailUser: gmail.email, gmailAppPassword: gmail.appPassword
      });

      if (!isOwner(userId)) decreaseUserFixLimit(userId);
      saveHistory({ user_id:userId, username, command:`/fix ${number}`, number_fixed:number, email_used:gmail.email, details:`messageId: ${apiRes.messageId}` });

      const country = detectCountry(number);
      const now = new Date().toLocaleString('id-ID', {timeZone:'Asia/Jakarta'});
      const sisa = isOwner(userId) ? '∞' : getUserFixLimit(userId).fix_limit;

      const channelMsg = `<b>📨 Fix WhatsApp</b>\n\n👤 @${username}\n🆔 ${userId}\n📱 +${number}\n🌍 ${country}\n📧 ${gmail.email}\n🕒 ${now}\n🔢 Sisa: ${sisa}x`;
      try { await bot.sendMessage(NOTIFICATION_CHANNEL_ID, channelMsg, {parse_mode:'HTML'}); } catch(e) { console.error('channel:', e); }
